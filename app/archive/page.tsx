...
export default function ArchivePage() {
  ...
  const handleOpenEditModal = (doc: Document) => {
    setEditingDoc(doc);
    setEditForm({ file_name: doc.file_name, category: doc.category || '기타' });
          ...
          <div className="bg-white rounded-[40px] w-full max-w-2xl shadow-2xl border border-white overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <form onSubmit={handleUpdateDoc} className="p-10 space-y-8 text-sm">
              ...
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setShowEditModal(false)} className="btn-secondary flex-1 h-14">취소하기</button>
                <button type="submit" className="btn-primary flex-[2] h-14 text-lg">자료 정보 업데이트</button>
              </div>
            </form>
          </div>
        </div>
